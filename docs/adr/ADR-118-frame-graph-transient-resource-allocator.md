...
- **Dispatch 120 — `TexturePool` + `BufferPool` MVP per D3 / D4 / D5.** NEW `crates/gfx/src/frame_graph/allocator.rs` (or `transient.rs` — file-shape call); per-frame-in-flight pool instances; descriptor-keyed reuse with aliasing-group identity; greedy max-size policy. Footprint ~250-400 LOC including unit tests covering pool reuse + ring-buffer rotation + descriptor matching. NO `wgpu::Device` actually called in unit tests (mock the descriptor → physical-handle surface); integration smoke covers real wgpu pool behavior.
