...
- NO pool implementation in dispatch 119. Dispatch 120 lands `TexturePool` (D3 / D4 / D5); dispatch 121 lands `BufferPool` (clean mirror); the integration with `FrameRecorder` is dispatch 122.
...
- **Phase 6 frame-graph chapter unblocked.** The remaining substrate work is mechanically derivable from this ADR's decisions: dispatch 119 lands descriptors, 120 lands `TexturePool`, 121 lands `BufferPool` (clean mirror), 122 wires the pools to `FrameRecorder`. No further design pressure between here and the chapter's exit gate.
...
4. **Specific shader / pipeline integration with `FrameRecorder` / `LitMeshPipeline`.** That is `gfx::intent_adapter`'s and `record_lit_mesh_pass`'s lane; the allocator hands physical handles to those layers, and they orchestrate the per-pass recording. Dispatch 122 implements this wiring; this ADR does not pin it.
...
9. **External-input resources (e.g. swapchain back-buffer as a frame-graph resource).** Substrate-level non-goal per `frame_graph::mod.rs:39`; the allocator does NOT manage swapchain integration. That is dispatch 122+'s concern when `FrameRecorder` integration lands.
10. **Determinism of pool free-list ordering.** The pool's internal free-list ordering is an implementation detail of the pool dispatches (120 / 121); the ADR pins descriptor identity and reuse contracts, NOT the free-list's data structure choice.
...
- **Dispatch 120 — `TexturePool` MVP per D3 / D4 / D5; dispatch 121 — `BufferPool` MVP (clean mirror).** NEW `crates/gfx/src/frame_graph/{texture_pool,buffer_pool}.rs`; per-frame-in-flight pool instances; descriptor-keyed reuse with aliasing-group identity; greedy max-size policy. Footprint ~250-280 LOC each (TexturePool 278L; BufferPool 268L) including unit tests covering pool reuse + ring-buffer rotation + descriptor matching. Pool unit tests are GPU-gated via the `ctx_or_skip!` macro: they use a real `wgpu::Device` when available and skip gracefully on headless CI (mirroring the `lit_mesh_pipeline.rs` test pattern).
- **Dispatch 122 — `ResourceMap` builder + `AliasingGroup::max_descriptor` analytical helper.** Provides the per-frame `ResourceId → Arc<wgpu::Texture> / Arc<wgpu::Buffer>` mapping that enables transient-resource consumption from pass-record sites when they grow such consumers (post-Phase 6 exit gate). Footprint ~150-200 LOC. `FrameRecorder` + `record_lit_mesh_pass` are intentionally untouched — today's pass-record sites have no transient-resource consumers; the integration with them is future work. **LOAD-BEARING `acquire()` dedup contract**: per `TexturePool` / `BufferPool` module docs, repeated `acquire(...)` calls with the same `(descriptor, AliasingGroupId)` within a single frame return DISTINCT allocations (no internal dedup). The `build_resource_map` entry point structurally enforces this contract by iterating `CompiledFrameGraph::aliasing_groups()` and calling `acquire(...)` exactly once per group; pass-record sites read from the `ResourceMap` only — never call `acquire(...)` directly. Otherwise aliasing-group reuse correctness is defeated.
- **Phase 6 umbrella exit gate.** The 60 fps simple-scene golden gate at `IMPLEMENTATION.md:468` becomes the chapter's exit criterion once dispatch 122 lands. The frame-graph allocator + ADR-117 handoff together make this measurable.
