...
- **Tier-B incremental algorithms.** Each Tier-B metric needs a per-metric partial-recomputation strategy; some (SCC count) are research-grade rather than mechanical. **Phase-2 fanout subset shipped 2026-05-10** inside `kernel/graph-foundation::Graph<N, E>` directly (mirrors phase-1's substrate placement; `tools/graph-metrics/src/main.rs` stub stays empty per sub-decision 1) — 5 public methods (`node_in_degree` / `node_out_degree` / `max_in_fanout` / `max_out_fanout` / `average_fanout`) + 9 acceptance tests + 2 cached `u32` fields + 2 partial-recomputation helpers (`recompute_max_in_fanout` / `recompute_max_out_fanout`). Per-mutation cost: insert is O(1) (the new src/dst degree is the only candidate that can become the new max); remove is O(1) when the affected node was NOT holding the cached max (typical case; cached value provably still attainable by an untouched node), else O(N_with_outgoing/incoming) via the recomputation helpers. The harder Tier-B metrics — `max_depth` / `scc_count` / `dependency_diameter` (and ADR-098's deferred `topology_lineage_breadth`) — are explicitly **not naturally incremental** and deserve their own **phase-2.5 dispatch** with algorithmic design discussion before implementation: `max_depth` requires DFS + cycle handling and adding an edge can change depth of many descendants; `scc_count` requires Tarjan/Kosaraju and is not naturally incremental at all; `dependency_diameter` is O(V*(V+E)) full BFS-from-each-node. A sub-ADR for the SCC-incremental strategy specifically may still be warranted.
