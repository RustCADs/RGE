...
> *[AMENDED 2026-05-10 — see Amendment below: `max_depth` / `SCC count` / `dependency diameter` / `topology lineage breadth` reclassified Tier-B → Tier-C as binding doctrine. The fanout subset (`average fanout` / `node_in_degree` / `node_out_degree` / `max_in_fanout` / `max_out_fanout`) remains Tier-B and is the only example set that survived the reclassification.]*
...
├── structural/   — Tier-B structural metrics (fanout subset only post-amendment)
├── topology/     — topology-volatility (cad-core domain; lineage-breadth Tier-C post-amendment)
├── invalidation/ — invalidation-propagation pressure metrics
├── recovery/     — probabilistic recovery confidence + repair-ambiguity
└── analytics/    — Tier-C: depth / SCC / diameter / lineage-breadth / entropy / centrality / prediction
```

> *[AMENDED 2026-05-10 — see Amendment below: depth / SCC / diameter (originally listed in `structural/`) and lineage-breadth (originally listed in `topology/`) reclassified Tier-C, so they live in `analytics/` per the amendment's "Implementation guidance for Tier-C" rule 1. The diagram above reflects the post-amendment intent.]*
...
**Phase 2 — Structural incremental metrics.** Scope: `structural/` sub-module. Metrics originally cited: `max_depth`, `average_fanout`, `scc_count`, `dependency_diameter` *[AMENDED 2026-05-10 — see Amendment below: `max_depth` / `scc_count` / `dependency_diameter` reclassified Tier-C; only `average_fanout` (and the broader fanout subset) remains Tier-B]*. Update path: partial recomputation around the mutation's affected region (NOT full-graph scan). Acceptance criterion: per-mutation cost on a 1k-node-DAG is ≤O(log n) for at least three of four metrics (the SCC metric may require O(n) on certain mutation classes — documented per-metric); soak test asserting the cost ceiling. Cross-references: ADR-098 (lineage breadth deferred to phase 3); kernel/graph-foundation::Graph adjacency caches. **Phase 2 fanout subset shipped 2026-05-10** (`average_fanout` plus per-node `node_in_degree` / `node_out_degree` plus workspace-wide `max_in_fanout` / `max_out_fanout`) inside `kernel/graph-foundation::Graph<N, E>` directly; the remaining three (`max_depth` / `scc_count` / `dependency_diameter`) were deferred and subsequently **reclassified Tier-C per the 2026-05-10 Amendment below** (no longer "phase-2.5 incremental" work — see Amendment for the binding analytical / on-demand classification).
...
- **Tier-B incremental algorithms.** Each Tier-B metric needs a per-metric partial-recomputation strategy. **Phase-2 fanout subset shipped 2026-05-10** inside `kernel/graph-foundation::Graph<N, E>` directly (mirrors phase-1's substrate placement; `tools/graph-metrics/src/main.rs` stub stays empty per sub-decision 1) — 5 public methods (`node_in_degree` / `node_out_degree` / `max_in_fanout` / `max_out_fanout` / `average_fanout`) + 9 acceptance tests + 2 cached `u32` fields + 2 partial-recomputation helpers (`recompute_max_in_fanout` / `recompute_max_out_fanout`). Per-mutation cost: insert is O(1) (the new src/dst degree is the only candidate that can become the new max); remove is O(1) when the affected node was NOT holding the cached max (typical case; cached value provably still attainable by an untouched node), else O(N_with_outgoing/incoming) via the recomputation helpers. The originally-Tier-B-cited metrics `max_depth` / `scc_count` / `dependency_diameter` (and ADR-098's `topology_lineage_breadth`) were **reclassified Tier-C per the 2026-05-10 Amendment below** — they are not naturally incremental (`max_depth` requires DFS + cycle handling and adding an edge can change depth of many descendants; `scc_count` requires Tarjan/Kosaraju and is not naturally incremental at all; `dependency_diameter` is O(V*(V+E)) full BFS-from-each-node), and the amendment formalized this as binding analytical / on-demand classification rather than a deferred phase-2.5 incremental sprint. See the Amendment's "Implementation guidance for Tier-C" for binding rules when consumers materialize.
