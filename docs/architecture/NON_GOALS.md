...
- **34 rollout-debt stub crates** — Tier-2 + Tier-1 failure-class exemptions per HANDOFF.md. The list: `anim-*` / `asset-pipeline` / `brep-render` / `build-pipeline` / `cad-native` / `cad-occt` / `errors` / `gfx-ir` / `hot-reload-watcher` / `input-gestures` / `io-{audio,obj,step,stl}` / `marketplace` / `marketplace-server` / `material-{graph,graph-editor,runtime}` / `math` / `physics-debug` / `plugin-discovery` / `replication` / `resources` / `script-{aot,graph}` (31 Tier-2) + `kernel/{shared, asset-view, asset-streaming}` (3 Tier-1). `kernel/io-scheduler` and `kernel/job-system` are no longer empty; both are PARTIAL v0 cavities with declared failure classes. Each remaining stub lib.rs is verbatim `//! \`rge-<name>\` — stub crate. Architecture frozen at v0.8; implementation pending per IMPLEMENTATION.md.`
...
- **94 workspace members; 43 IMPLEMENTED / 5 PARTIAL / 46 EMPTY-STUB** — the EMPTY-STUB count IS the non-goals enumeration in concrete form.
- **Tier 1 kernel: 10 of 15 implemented + 2 PARTIAL v0 cavities** — 3 stubs (`shared`, `asset-view`, `asset-streaming`) remain on the deferral list; `kernel/io-scheduler` and `kernel/job-system` now have vocabulary substrate but intentionally minimal behavior.
