...
- **33 rollout-debt stub crates** — Tier-2 + Tier-1 failure-class exemptions per HANDOFF.md. The list: `anim-*` / `asset-pipeline` / `brep-render` / `build-pipeline` / `cad-native` / `cad-occt` / `errors` / `gfx-ir` / `hot-reload-watcher` / `input-gestures` / `io-{audio,obj,step,stl}` / `marketplace` / `marketplace-server` / `material-{graph,graph-editor,runtime}` / `math` / `physics-debug` / `plugin-discovery` / `replication` / `resources` / `script-{aot,graph}` (31 Tier-2) + `kernel/{shared, asset-streaming}` (2 Tier-1). `kernel/io-scheduler`, `kernel/job-system`, and `kernel/asset-view` are no longer empty; all three are PARTIAL v0 cavities with declared failure classes. Each remaining stub lib.rs is verbatim `//! \`rge-<name>\` — stub crate. Architecture frozen at v0.8; implementation pending per IMPLEMENTATION.md.`
...
- **94 workspace members; 43 IMPLEMENTED / 6 PARTIAL / 45 EMPTY-STUB** — the EMPTY-STUB count IS the non-goals enumeration in concrete form.
- **Tier 1 kernel: 10 of 15 implemented + 3 PARTIAL v0 cavities** — 2 stubs (`shared`, `asset-streaming`) remain on the deferral list; `kernel/io-scheduler`, `kernel/job-system`, and `kernel/asset-view` now have vocabulary substrate but intentionally minimal behavior.
