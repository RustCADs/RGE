...
- **View / Play menu breadth** remains unbuilt — each needs a NEW action first (View → Reset Camera, e.g. via `isometric_camera_for_bounds`; Play → a clickable control routing to the already-runtime-wired `handle_button`). The A2 scoping criterion was: wire only items with BOTH a `Command` variant AND a reachable shell action — Undo/Redo were the only two that qualified.
