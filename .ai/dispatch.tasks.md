...
154. **Add tracked-CAD-entity liveness fields to `CadSceneInspection` (editor-shell only).**
   Extend the existing read-only headless CAD scene inspection snapshot so a
   future UI caller or headless test can distinguish "the shell still stores a
   tracked CAD entity id" from "that tracked entity is still live and renderable
   through the existing projection mesh path." Do not add UI wiring, cleanup
   policy, CommandBus routing, undo/dirty/save behavior, render refresh, GPU
   upload, CAD-core behavior, or cad-projection behavior.

   The bounded target is an inspection API extension only. Add explicit
   read-only fields to `CadSceneInspection` with names such as
   `tracked_cad_entity_live` and `tracked_cad_entity_render_mesh_present` (the
   exact names may be adjusted if the implementation proves a tighter equivalent).
   The fields must be computed from the existing `cad_entity`, `cad_world`,
   `BRepHandle`, and `CadProjection::render_mesh_for` state without ticking
   projection, despawning entities, mutating selection, initializing render
   state, uploading meshes, or touching GPU resources.

   **Self-re-arm (final step, required):** After the implementation is complete
   and verified, follow the Self-re-arm protocol in this file's header: append
   exactly one bounded next-task-source AUDIT task as task 155 - a
   source/docs-read-only audit of the post-tracked-CAD-liveness inspection
   boundary - or, if no bounded in-policy next task exists, append a single
   `NEEDS_HUMAN_RECORDED: <ISO-date> - <reason>` line instead. The task 155
   audit must require dispatcher-provided GitHub-state snapshot evidence for
   queue/already-filed-task claims, must compare current source before choosing
   the next feature, and must itself carry this Self-rearm final-step
   requirement. Edit `.ai/dispatch.tasks.md` to do this.
   ...
   **MAY edit:**
   - `crates/editor-shell/src/lifecycle/commands.rs`
   - `crates/editor-shell/src/lifecycle/tests.rs`
   - `crates/editor-shell/src/lifecycle/mod.rs` only if the implementation needs
     an adjacent lifecycle-module doc/re-export adjustment for the public
     inspection API
   - `.ai/dispatch.tasks.md`
   - generated ISSUE-<n> handoff/audit/log artifacts for the dispatch
   ...
   **Done criteria:**
   ...
   - `EditorShell::cad_scene_inspection(&self)` remains a pure read: it does not
   ...
   - Focused lifecycle tests cover a fresh shell, the successful first cuboid
     add, rejected non-empty add preserving the inspection snapshot, and
     `restore_to(pre_add_head)` plus `despawn_brep_entity` cleanup. The cleanup
     case must show the stored tracked entity id can still be present while the
     new liveness/render-mesh fields report false.
   - No menu item, shortcut, command-palette entry, UI/host route, plugin
     command, CommandBus `Action`, global undo/redo, dirty/save-mark,
     save/load/open/close behavior, camera behavior, viewport behavior,
     render-path refresh, CAD-core behavior, or cad-projection behavior is
   ...
   **Verification:**
   ...
   - `rg -n "CommandBus|Action|submit\\(|undo_command|redo_command|mark_saved_command" crates/editor-actions/src crates/editor-shell/src/lifecycle/commands.rs` expected read-only confirmation only; no diff under `crates/editor-actions/**`
   - `rg -n "^153\\.|^154\\.|^155\\.|NEEDS_HUMAN_RECORDED" .ai/dispatch.tasks.md`
   - `git diff --name-only`
   - `git diff --check`

   **Halt conditions:**
   - The liveness/renderability fields cannot be computed from existing
     `cad_entity` / `cad_world` / `BRepHandle` / `render_mesh_for` state without
   ...
   - The implementation would need to clean up or mutate CAD graph/projection/
     world, selection, render assets, camera, CommandBus, save source, or dirty
     state while answering the inspection query.
   - Making the fields meaningful requires broader cleanup policy, multi-root
     CAD composition, deletion, transforms, parameter editing, persistence, UI
     wiring, command routing, or render refresh work.
   - Tests require constructing real `wgpu` resources instead of staying
     strictly headless.
...
155. **[DONE 2026-06-15 via ISSUE-410 - selected task 156 stale tracked CAD entity cleanup helper] Audit post-tracked-CAD-liveness inspection boundary.**
   ...
   dispatcher-provided GitHub-state snapshot copied into the issue body (or an
   exact local artifact copied from it), then choose exactly one smallest bounded
   FEATURE task that can safely follow the tracked-CAD liveness/render-mesh
   inspection boundary, or record `NEEDS_HUMAN_RECORDED` if every candidate
   crosses a policy or architecture boundary. Do not use live `gh`, browser,
   network, or GitHub API access for queue-state or already-filed-task evidence.

   Compare current source before naming a next feature. Candidate classes to
   inspect include: lifecycle-only cleanup or consistency around stale tracked
   CAD entity ids after graph restore/projection despawn; making the inspection
   snapshot useful to a future UI caller without adding UI wiring yet;
   source/test-only gaps exposed by the new tracked-CAD liveness fields; and any
   smaller post-inspection CAD/editor-shell boundary visible in current source.
   The selected feature must be one bounded dispatch, not a batch or roadmap
   item.

   **Self-re-arm (final step, required):** After the audit is complete and
   verified, follow the Self-re-arm protocol in this file's header: append
   exactly one bounded FEATURE task as task 156, carrying this same
   Self-rearm final-step requirement so the chain continues, or, if no bounded
   in-policy feature exists, append a single
   `NEEDS_HUMAN_RECORDED: <ISO-date> - <reason>` line instead. Edit
   `.ai/dispatch.tasks.md` to do this.
   ...
   **Done criteria:**
   - The audit records the pre-edit task-heading check for
     `^154\.|^155\.|^156\.` and proves no task 156 heading existed before
     self-rearm.
   - Queue/already-filed-task claims cite only the dispatcher-provided snapshot
     embedded in the issue body or an exact local artifact path copied from it;
     no live `gh`, browser, network, or GitHub API query is run by the sandboxed
   ...
   - Current source/docs are read before selecting the next feature, and each
     negative or unchanged current-state claim has a falsifying search recorded
     where practical.
   - Exactly one bounded FEATURE task 156 is appended with explicit `MAY edit`,
     `MUST NOT edit`, `Done criteria`, `Verification`, and `Halt conditions`,
     and it carries the copied Self-rearm final-step requirement, or a
     source-grounded `NEEDS_HUMAN_RECORDED` record is written.
   - No implementation work for task 156 is done, and no other task is added.

   **Verification:**
   - `rg -n "^154\.|^155\.|^156\.|NEEDS_HUMAN_RECORDED" .ai/dispatch.tasks.md`
     before edits and after edits
   - source/docs greps recorded in the audit, including the post-tracked-CAD
     liveness lifecycle/CAD boundary inspected before choosing the next feature
   - dispatcher-snapshot read recorded from the local issue-body artifact; no
     live GitHub/network query
   - `git diff --name-only`
   - `git diff --check`
...
156. **[DONE 2026-06-16 via ISSUE-411 - appended task 157 human-gated post-cleanup audit pause] Clear stale tracked-CAD entity ids through a lifecycle-only helper (editor-shell only).**
...
157. **GATED AUDIT: record human decision point after stale tracked-CAD cleanup.**
   This is a SOURCE/DOCS READ-ONLY AUDIT ONLY. Read current source/tests/docs
   plus the dispatcher-provided GitHub-state snapshot copied into the issue
   body, compare the post-stale-tracked-CAD-cleanup boundary, and stop by
   recording a human decision marker. This task MUST NOT append task 158, MUST
   NOT append any feature or implementation task, and MUST NOT self-re-arm with
   a follow-up feature.

   Dispatcher snapshot evidence for queue/already-filed-task claims must come
   from the task packet or exact local issue-body artifact. For the ISSUE-411
   chain, the available dispatcher-provided snapshot was generated by
   ...
   autonomous issues were `(none)`, and already-filed autonomous issues were
   the provided closed `ai-auto` issue list through ISSUE-410. Do not replace
   that evidence with live `gh`, browser, network, or GitHub API access.

   Compare current source before making any recommendation. Inspect only the
   smallest relevant boundary around `EditorShell::clear_stale_tracked_cad_entity`,
   `CadSceneInspection`, tracked CAD entity liveness, render-mesh inspection
   data, graph restore/projection despawn cleanup tests, and adjacent docs or
   task-brief provenance needed to explain the next human decision. Negative or
   unchanged source claims must carry a falsifying `rg`/read command and result.

   **Self-rearm final step - BRIEF-ENFORCED PAUSE (operator decision):**
   After the audit is complete and verified, edit `.ai/dispatch.tasks.md` only
   to record exactly one marker line:
   `NEEDS_HUMAN_RECORDED: YYYY-MM-DD - source-grounded reason`
   using the actual audit date and a source-grounded reason. Immediately follow
   that marker with a `Recommendation for human approval` block covering:
   proposed next feature, exact edit surface, risks, verification, and why that
   is the smallest coherent next step. Do not append task 158. Do not append a
   feature task. Carry this same gated-audit instruction into the EXEC packet.
   ...
   **Done criteria:**
   - The audit records the pre-edit task-heading check for
     `^156\.|^157\.|^158\.` and proves no task 158 heading exists before the
     final marker edit.
   - Queue/already-filed-task claims cite only dispatcher-provided snapshot
     evidence from the issue body/task packet or exact local artifact; no live
     `gh`, browser, network, GitHub API, or web lookup is run.
   - Current source/tests/docs are read before the recommendation is written,
     and each practical negative or unchanged source claim has a falsifying
     search/read recorded.
   - Exactly one `NEEDS_HUMAN_RECORDED: YYYY-MM-DD - source-grounded reason`
     marker line is recorded with the actual audit date, immediately followed by
     a `Recommendation for human approval` block covering proposed next feature,
     exact edit surface, risks, verification, and why it is the smallest
     coherent next step.
   - No task 158, no feature/implementation task, and no source/docs/Cargo/
     workflow/script/schema changes are added.

   **Verification:**
   - `rg -n "^156\.|^157\.|^158\.|NEEDS_HUMAN_RECORDED" .ai/dispatch.tasks.md`
     before edits and after edits
   - source/docs greps recorded in the audit, including the
     post-stale-tracked-CAD-cleanup lifecycle/CAD boundary inspected before the
     recommendation is written
   - dispatcher-snapshot read recorded from the local task packet or issue-body
     artifact; no live GitHub/network query
   - `git diff --name-only`
   - `git diff --check`
   - `.\new-handoff.ps1 -Finalize -PacketPath <EXEC_PACKET> -DryRun`
   ...
   **Halt conditions:**
   ...
   - A recommendation cannot be made without resolving more than one coherent
...
159. **GATED AUDIT: record human decision point after stale-id preflight normalization.**
   This is a SOURCE/DOCS READ-ONLY AUDIT ONLY. Audit the post-stale-id
   first-cuboid preflight-normalization boundary after task 158, compare current
   local source/docs/backlog state, and stop by recording a human decision
   marker. This task MUST NOT append task 160 or any feature/implementation
   task, and MUST NOT self-re-arm with a follow-up feature.

   Dispatcher-provided GitHub-state snapshot evidence for queue/already-filed
   autonomous-task claims MUST come from the local task packet or exact local
   issue-body artifact. Do not call `gh`, browser, network, the GitHub API, or
   any live remote lookup for that confirmation. Preserve these snapshot facts
   in the audit:
   - Snapshot generated by `Invoke-AiDispatchAuto.ps1` at
     `2026-06-16T10:32:46.6703711+03:00`, before GitHub issue #414 was created.
   - Repo: `RustCADs/RGE`.
   - Open `ai-dispatch` issues before #414 was created: none.
   ...
   - Relevant already-filed autonomous issues include #403, #405, #406, #407,
     #408, #409, #410, #411, and #412 closed; current issue #414 was filed
     after this snapshot from task 158.

   Compare current local source/docs/backlog state before concluding. Inspect
   only the smallest relevant boundary around
   `EditorShell::add_cad_cuboid_to_empty_scene`,
   `EditorShell::clear_stale_tracked_cad_entity`, `CadSceneInspection`, tracked
   CAD entity liveness, render-mesh inspection data, first-cuboid rejection
   behavior, and adjacent task-brief provenance needed to explain the next human
   decision. Negative or unchanged source/docs/backlog claims must carry a
   falsifying `rg`/read command and observed result.

   **Self-rearm final step - BRIEF-ENFORCED PAUSE (operator decision):**
   After the audit is complete and verified, edit `.ai/dispatch.tasks.md` only
   to record exactly one marker line:
   `NEEDS_HUMAN_RECORDED: YYYY-MM-DD - source-grounded reason`
   using the actual ISO date and a concrete source-grounded reason. Immediately
   follow that marker with a `Recommendation for human approval` block covering:
   proposed next feature, exact edit surface, risks, verification, and why that
   proposal is the smallest coherent next step. Do not append task 160. Do not
   append a feature task. Carry this same gated-audit instruction into the EXEC
   packet.

   **MAY edit:**
   - `.ai/dispatch.tasks.md` only for the final `NEEDS_HUMAN_RECORDED` marker
     and immediately following `Recommendation for human approval` block
   - generated current-dispatch handoff/audit/log artifacts for the audit
   ...
   **Done criteria:**
   - The audit records the pre-edit task-heading check for
     `^158\.|^159\.|^160\.` and proves no task 160 heading or unresolved
     implementation-dispatch `NEEDS_HUMAN_RECORDED` marker exists before the
     final marker edit.
   - Queue/already-filed-task claims cite only dispatcher-provided snapshot
     evidence from the task packet or exact local issue-body artifact; no live
     `gh`, browser, network, GitHub API, or web lookup is run.
   - Current local source/docs/backlog state is read before the recommendation
     is written, and each practical negative or unchanged claim has a
     falsifying search/read recorded.
   - Exactly one `NEEDS_HUMAN_RECORDED: YYYY-MM-DD - source-grounded reason`
     marker line is recorded with the actual ISO date, immediately followed by
     a `Recommendation for human approval` block covering proposed next feature,
     exact edit surface, risks, verification, and why that proposal is the
     smallest coherent next step.
   - No task 160, no feature/implementation task, and no source/docs/Cargo/
     workflow/script/schema changes are added.

   **Verification:**
   - `rg -n "^158\.|^159\.|^160\.|NEEDS_HUMAN_RECORDED" .ai/dispatch.tasks.md`
     before edits and after edits
   - local source/docs/backlog reads recorded in the audit, including the
     post-stale-id first-cuboid preflight boundary inspected before the
     recommendation is written
   - dispatcher-snapshot read recorded from the local task packet or issue-body
     artifact; no live GitHub/network query
   - `git diff --name-only`
...
- Exact edit surface: `crates/editor-actions/src/action.rs`, `crates/editor-actions/src/bus.rs`, and their focused tests for the minimal action-context extension; `crates/editor-shell/src/lifecycle/commands.rs`, `crates/editor-shell/src/lifecycle/tests.rs`, and `crates/editor-shell/src/lifecycle/mod.rs` only if an adjacent shell export is required. Do not edit CAD-core, cad-projection, render-path, UI/host, Cargo metadata, workflows, dispatch automation, schemas, or packet templates for that feature.
